...
if __name__ == "__main__":
    ...
    for epoch in range(1, n_epochs+1):
        ...
        # Save model state
        model_state_path = os.path.join(results_dir, f'model_state_at_epoch_{epoch}')
        ...
        # Save rll and tll
        rll_df_path = os.path.join(results_dir, f'rll_at_epoch_{epoch}.csv')
        rll_df = pd.DataFrame(rll, columns=['Epoch', 'Iteration', 'TRL', 'VRL', 'PRL', 'FRL'])
        rll_df.to_csv(rll_df_path, index=False)

        tll_df_path = os.path.join(results_dir, f'tll_at_epoch_{epoch}.csv')
